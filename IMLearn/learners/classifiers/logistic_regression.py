...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "l1":
            self.coefs_ = self.solver_.fit(
                RegularizedModule(LogisticModule(), L1(), self.lam_, init, self.include_intercept_), X, y)
        elif self.penalty_ == "l2":
            self.coefs_ = self.solver_.fit(
                RegularizedModule(LogisticModule(), L2(), self.lam_, init, self.include_intercept_), X, y)
        else:
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        return np.where(self.alpha_ < x, 1, 0)
